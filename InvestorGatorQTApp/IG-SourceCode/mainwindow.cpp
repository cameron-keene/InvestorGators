...
//    for(int i = 0; i < this->stocks1.size(); i++){
//        std::string str = stocks1[i].GetName();
//        displayString = QString::fromStdString(str);
...
vector<DailyStock> Search(string symbol, Date d1, Date d2, vector<DailyStock> fullArr) {
    ...
    if (symbol == "")
    {
        ...
        for (int i = 0; i < fullArr.size(); i++) {
          // cout << fullArr[i].GetSymbol() << " " << fullArr[i].GetDate().GetMonth() << fullArr[i].GetDate().GetDay() << fullArr[i].GetDate().GetYear()<< endl;
            if(symbol == fullArr[i].GetSymbol())
            {
                // checks if in a valid year
                if (d1.GetYear() <= fullArr[i].GetDate().GetYear() && d2.GetYear() >= fullArr[i].GetDate().GetYear()) {
                    // checks if in one of the edge years
